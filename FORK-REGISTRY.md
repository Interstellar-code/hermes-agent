...
| **hermes-switch-ui** | SwitchUI integration: `switchui_info`/`switchui_status` tools, `pre_llm_call` first-LLM-call nudge, `switchui` skill | `plugins/hermes-switch-ui/` (6 commits MB..main) | `9662b85c5f`, `f0b4861876`, `79868bf874`; serialization wrapper `7ad272fea2` | FORK-ONLY; dir absent at tag | 2026-09-11 | **MEDIUM (BFF/web_server seam)**: tools query live gateway/dashboard/BFF state (ports 3002/8642/9119); upstream churned 227 `hermes_cli` paths incl. `web_server.py` (`c0ff25a1f8`, post-release). Replay clean; re-verify what the tools observe post-adopt. **⚠ SECURITY, found 2026-09-19 (S-1):** `dashboard/plugin_api.py:29` `_require_auth()` imports `hermes_cli.web_server._is_authenticated` — a name with **no definition anywhere** (fork tree, merge-base `3ef6bbd201`, or tag `345cd2b057`). The `except (ImportError, AttributeError): pass` swallows the guaranteed ImportError, so the guard returns None = allow. `Depends(_require_auth)` on `/connection`, `/register`, `/settings`, `/status`, `/heartbeat` is decorative **today**, not post-adopt. Public replacement `_require_token()` exists on both sides (fork `web_server.py:485`, tag `web_server.py:417`). Fix is a fail-open→fail-closed change — needs a human decision, see §4. |
...
| **Plugin dashboard auth guard (S-1) — AWAITING USER DECISION** | Two fork plugins gate their dashboard routes on a function that does not exist, so the gate always allows | `plugins/hermes-switch-ui/dashboard/plugin_api.py:29`, `plugins/personas/dashboard/plugin_api.py:33` | n/a — **deliberately not fixed in this PR**; the fix is a behavior change, not an import repoint | Not an upstream defect: `_is_authenticated` has **no definition** at merge-base `3ef6bbd201` or tag `345cd2b057` either — the name was never real | 2026-09-19 | **Live defect, independent of the adopt.** Demonstrated: recompiling each real `_require_auth` with `hermes_cli.web_server` blocked returns `None` (= allow) for both; repo-wide `def _is_authenticated` = 0. Affects `/connection`, `/register`, `/settings`, `/status`, `/heartbeat` (switch-ui) + the personas routes. **Options:** (a) repoint to `_require_token()`, which exists on both sides (fork `web_server.py:485`, tag `web_server.py:417`) and raises 401 — smallest diff, but flips fail-open→fail-closed and may 401 a loopback dashboard whose token plumbing differs; (b) keep fail-open but make it loud (log a warning once) — no lockout risk, no security gain; (c) vendor a real check into the plugins. **Recommend (a)**, gated on the user confirming a dashboard-login smoke test, because a silent no-op guard is worse than either outcome. Do not ship (a) blind — it can lock the user out of their own dashboard. |
