...
| **Multi-profile cron ticking — NOT PORTED, structural** | Fork splits `tick()` into a per-profile `_tick_single_profile` plus a `tick()` wrapper looping `profiles_to_serve(multiplex=True)` under `set_hermes_home_override`, so one scheduler serves every profile | `cron/scheduler.py` | (part of the fork's +207 to this file) | **OURS, and NOT portable as a diff** — upstream independently restructured `tick()` far more heavily: persistent worker pool, `_should_yield_tick_to_fresh_gateway` skew gate, `_maybe_run_worktree_maintenance`, ESTOP check, external-worker handoff dispatch, `_sweep_mcp_orphans` | 2026-09-20 | **DECISION NEEDED — not a port, a redesign.** There is no seam to graft the fork's profile loop onto; landing it means re-deriving how per-profile ticking composes with the parallel worker pool, the tick lock, and the external-worker adoption path. Deliberately skipped in Phase D unit 19 (`fa36863139`) rather than half-applied. **Not silently broken:** `tests/cron/test_multiplex_cron_scheduler.py::test_multiplex_cron_tick_serves_all_profiles` fails IDENTICALLY on the pre-port baseline (proved by cp-swap, not `git checkout`), so this is a known pre-existing gap, not a regression. Ties directly to the parked multiplex-gateway direction — decide that first, then size this. Until then **cron serves the default profile only.** |
| **Profile / multiplex fixes** | Fail closed on ignored `/p/<profile>/` prefix; reconcile drifted profile schemas on read-only open; resolve profile cutover blockers | `gateway/platforms/api_server.py`, `gateway/platforms/webhook.py`; `hermes_state.py`; `cron/scheduler.py`, `gateway/config.py`, `gateway/run.py` | `eb8a6033eb` (#202), `994980a710` (#202), `86480a4c67` (#205) | All files ∈ 125-file ∩ (`api_server.py` = 22 fork commits touch) | 2026-09-20 | **SPLIT VERDICT — sized 2026-09-20 during Phase D.** **(a) Fail-closed `/p/<profile>/` prefix: CONVERGENT — DROP OURS.** Upstream ships it at `345cd2b057` (`gateway/platforms/webhook.py:394-415`) as a **superset**: fork rejects *any* prefix when multiplex is off; upstream rejects a *foreign* prefix but lets a **self-referential** one fall through via `profile_matches_home` — equally safe against serving one profile's routes under another's URL, without 404ing a gateway that addresses itself by name. All 16 multiplex tests pass on the adopted tree with zero fork code. **But upstream has no multiplex-off test** (its class has 3: roundtrip, no-prefix, unserved-prefix), so the fork's tests were ported and extended instead — `Phase D unit 4` (`64b6e614be`): drop our code, keep and widen our coverage. **(b) Profile-schema reconcile + cutover blockers: still OURS, unported** — re-derive against the tag's profile machinery (heavy: 216 `profile` mentions in tag `tui_gateway/server.py`); assert cross-profile isolation (Phase I soak cluster 1). |
...
| **`/goal` evaluate-and-continue** | Evaluate and continue a goal after a turn | `gateway/platforms/api_server.py`, `tests/gateway/test_session_api.py` | `68d49d19bc` (#231) | **OURS** — `/goal` and `goal` → 0 in tag `api_server.py` (2 `goal` refs live in the `api_server_runs.py` sibling) | 2026-09-11 | Replay in full (Phase D cluster 5 gate commit; §6.6 fork-owned surface). **LANDED 2026-09-23 (after being missed).** The Phase D port missed the SSE half: `goal.status`/`goal.continuation` were absent from HEAD until `fix/post-0.21.3-regressions`, found by a SwitchUI client review. Restored in `_run_and_signal`, reusing `hermes_cli.goals.GoalManager.evaluate_after_turn` (no second judge). **Overlap with upstream `/goal` (tui_gateway/methods_session_control.py): MERGE.** Both use `hermes_cli.goals` load/save as the one state store; upstream's slash is the control plane (snapshot/pause/resume), ours is the per-turn judge plus continuation on the HTTP stream. Nothing to drop. Lesson: the judge surviving in goals.py was read as "ported"; the client-visible SSE output was not checked. |
...
| **`/subgoal` dispatch** | tui_gateway dispatches `/subgoal` + advertises dispatchable bundles | `tui_gateway/server.py`, `tests/test_tui_gateway_server.py` | `cdaa6500dd` (#237) | ⚠ **CORRECTED 2026-09-19 — NO LONGER "OURS". Convergent evolution at v0.21.3.** The previous verdict ("`subgoal` matches 15 tag files but **none under `tui_gateway/`**") was **true at v0.20.6 `5fc308a707` and is FALSE at `345cd2b057`**: `subgoal` now matches **2 files under `tui_gateway/`** — `methods_session_control.py` (RPC methods `subgoal.add` / `subgoal.remove` / `subgoal.clear`, `"subgoals": list(state.subgoals)`, and `_SESSION_CONTROL_SLASHES = frozenset({"goal","heartbeat","loop","subgoal"})` at L213-214) and `contracts/config_free_tier_control.py` (typed contract, L473-482). Upstream built the **same dispatch surface** the fork owns. | 2026-09-19 | **Re-size before replay — this is the second convergent-evolution case after run steering.** Do NOT blind-replay `cdaa6500dd` onto `tui_gateway/server.py`: upstream's dispatch lives in `methods_session_control.py`, a module that did not exist at v0.20.6. Compare the two contracts (fork's `/subgoal` dispatch + dispatchable-bundle advertisement vs upstream's three RPC methods + `_SESSION_CONTROL_SLASHES`) and decide DROP-OURS / MERGE / KEEP-OURS explicitly, as was done for steering in §2's run-steering row. The dispatchable-bundle advertisement may still be fork-only — verify it separately from the `subgoal` verb itself. |
