...
async def _ollama_context_preflight() -> None:
    ...
    NOTE: the natural call site for this (right before the startup-restore
    serialization) now lives in ``gateway/run_startup.py``'s
    ``GatewayStartupMixin.start()``, which is out of this port's assigned
    scope; this function is defined but not yet wired to a caller.
    """
    try:
