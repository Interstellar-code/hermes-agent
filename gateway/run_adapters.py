    ...
    async def _handoff_watcher(self, interval: float = 2.0, drain_timeout: float = 30.0) -> None:
        ...
        async def _tick(profile_name: Optional[str] = None) -> None:
            ...
            if session_db is None:
                return
            # FORK-ONLY (#221): retire requests nobody picked up in time BEFORE listing. A pending
            # row is a standing instruction to re-bind a session to a platform; without this sweep,
            # one abandoned when its requester was killed mid-wait would be executed by whichever
            # gateway started next — minutes or days later. Guarded on its own so a sweep failure
            # can never cost the tick its real work (claiming live handoffs).
            try:
                expired = await session_db.expire_stale_handoffs()
            except asyncio.CancelledError:
                raise
            except Exception:
                expired = []
